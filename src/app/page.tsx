...
export default function SupportLoginPage() {
  ...
  const [err, setErr] = useState<string | null>(null);
    ...
    try {
      ...
      if (userRole !== "support") {
        setErr("Bu panele sadece çağrı merkezi erişebilir.");
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center">
      <div className="w-full max-w-md bg-white/90 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-orange-200">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-orange-600">Çağrı Merkezi Paneli</h1>
          <p className="text-gray-600 mt-2">Çağrı merkezi giriş ekranı</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          >
            {loading ? "Giriş yapılıyor..." : "Çağrı Merkezi Giriş"}
          </button>
        </form>

        <p className="text-xs text-center text-gray-500 mt-6">
          Bu panel sadece çağrı merkezi kullanıcıları içindir.
        </p>
      </div>
    </div>
  );
