...
export default function CourierStatsPage() {
  ...
  const [statsMeta, setStatsMeta] = React.useState<CourierStatsMeta | null>(
    null
  );
  ...
  const deliveredCountNorm = React.useMemo(() => {
    if (!stats) return 0;
    const raw =
      stats.deliveredPackagesCount ?? stats.deliveredPackageCount ?? 0;
  ...
  const pkgDelivered =
    ...
    null;
  const pkgRemaining =
    pkgRaw?.remainingPackages ?? (pkgRaw as any)?.remaining_packages ?? null;
  const pkgHasLeft =
    pkgRaw?.hasPackageLeft ?? (pkgRaw as any)?.has_package_left ?? null;
  const pkgType = pkgRaw?.packageType ?? (pkgRaw as any)?.package_type ?? null;
  const pkgNote = pkgRaw?.note ?? null;
  const pkgUpdatedAt = pkgRaw?.updatedAt ?? (pkgRaw as any)?.updated_at ?? null;
  ...
  const lastStatusChangeValue =
    stats?.status?.lastStatusChange ?? (stats as any)?.lastStatusChange ?? null;
        ...
        <div className="mt-2 rounded-2xl border border-neutral-100 bg-neutral-50/70 p-4">
          ...
          {selectedId && !statsLoading && !statsError && stats && (
            <div className="space-y-4">
                ...
                <div>
                  ...
                  {selectedCourier && (
                    <div className="mt-1 text-xs text-neutral-600">
                      ...
                      {selectedCourier.phone && selectedCourier.email && (
                        <span> · </span>
                      )}
                ...
                {pkgRaw ? (
                  <div className="p-4 space-y-4">
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                      ...
                      <StatBox
                        title="Kalan Gün"
                        value={pkgRemainingDays ?? "-"}
                      />
                      <StatBox
                        title="Aktif Çalışılan Gün"
                        value={pkgActivityDays ?? "-"}
                      />
                      <StatBox
              ...
              {/* ekstra alanlar */}
              {/* Ekstra Bilgiler (Dinamik Alanlar) */}
              {extraEntries.length > 0 && (
                <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden">
                  <div className="bg-neutral-50/80 border-b px-4 py-3 flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-400 bg-white px-2 py-0.5 rounded-md border border-neutral-200">
                      {extraEntries.length} Veri
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                    {extraEntries.map(([key, value], idx) => (
                      <div
                        key={key}
                        className={`p-4 flex flex-col gap-1 border-neutral-100 ${
                          // Kenarlık tasarımı: Hücreler arası ince çizgiler
                          idx % 3 !== 2 ? "lg:border-r" : ""
                        } ${
                          idx <
                          extraEntries.length - (extraEntries.length % 3 || 3)
                            ? "border-b"
                            : ""
                        }`}
                      >
                        <dt className="text-[11px] font-bold text-neutral-400 uppercase tracking-tight">
                          {key.replace(/([A-Z])/g, " $1").replace(/_/g, " ")}
                          {/* CamelCase veya snake_case'i insancıl metne çevirir */}
                        </dt>
                        <dd
                        ...
                        >
                          {typeof value === "boolean" ? (
                            value ? (
                              ...
                              <span className="text-neutral-400 bg-neutral-100 px-2 py-0.5 rounded text-xs">
                                Pasif
                              </span>
                            )
                          ) : typeof value === "object" ? (
                            <pre className="text-[10px] bg-neutral-50 p-1 rounded">
